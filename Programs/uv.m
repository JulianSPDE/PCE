function UV = uv(U,V,N,polynomialtype) % Input: vectors U, V of size p^d x (N+1) (column vectors!)
...
UV = zeros(pp*(N+1),1); % same length as U, V
for eta=0:N
    for l=0:N
        for m=0:l
                for p=0:min(l,m-l)
                    if eta==l-2*p
                        UV(u_part(pp,eta)) = UV(u_part(pp,eta)) + U(u_part(pp,m)).*V(u_part(pp,l-m)) ...
                            *C(m,l-m,p);
                    end
                end
        end
