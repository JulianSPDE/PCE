function [F_UV,tracker,trackernotzero] = uv2(U,V,N,polynomialtype) % Input: vectors U, V of size p^d x (N+1) (column vectors!)
            ...
            for j=0:m
                for p=0:min(j,m-j)
                    for n=0:min(l-m,m-2*p)
                        F_UV(u_part(pp,eta)) = F_UV(u_part(pp,eta)) + U(u_part(pp,l-m)).*V(u_part(pp,j)).*V(u_part(pp,m-j)) ...
